...
def try_select_installments(page, desired: int) -> dict:
    evidence = {"selected": False, "detail": None, "error": None}
    try:
        snap = page.evaluate(
            """
() => {
...
"""
        ...
        evidence["detail"] = snap
        for sel in snap:
            for opt in sel.get("options") or []:
                txt = (opt.get("text") or "").lower()
                if "cuota" in txt and str(desired) in txt:
                    locator = None
                    if sel.get("id"):
                        locator = page.locator(f"select#{sel['id']}")
                    elif sel.get("name"):
                        locator = page.locator(f"select[name='{sel['name']}']")
                    else:
                        continue
                    if locator.count() > 0:
                        locator.first.select_option(value=opt.get("value"))
                        evidence["selected"] = True
                        evidence["selected_selector"] = f"#{sel.get('id')}" if sel.get("id") else f"select[name='{sel.get('name')}']"
                        evidence["selected_value"] = opt.get("value")
                        evidence["selected_text"] = opt.get("text")
                        return evidence
    except Exception as e:
        evidence["error"] = f"{type(e).__name__}: {e}"[:240]
    return evidence
...
def main() -> None:
    ...
    installments_wanted = int(os.getenv("CERT_INSTALLMENTS") or "2")

    result = {
        "case": "05_authorize_installments",
        "started_at": now_iso(),
        "backend": BACKEND,
        "ok": False,
        "failure_step": None,
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                page.wait_for_selector("#card-number", timeout=90000)
            except Exception:
                page.screenshot(path=str(out_dir / "screenshots/FAIL_card_number.png"), full_page=True)
                raise
            ...
            sel_evidence = try_select_installments(page, installments_wanted)
            write_json(out_dir / "installments.selection.json", sel_evidence)
            result["installments_selected"] = sel_evidence
            ...
            result["artifacts"]["installments_selection"] = str(out_dir / "installments.selection.json")
    ...
    try:
        ...
        detail = None
    result["authorize"] = {"http_status": ar.status_code, "buy_order": auth_body.get("buy_order"), "detail": detail}
    ...
    if isinstance(detail, dict):
        ...
        installments = detail.get("installments_number")
        status = detail.get("status")

    if not (response_code == 0 and status == "AUTHORIZED" and isinstance(installments, int) and installments >= installments_wanted):
        result["failure_step"] = "installments not obtained"
        result["error"] = {"response_code": response_code, "status": status, "installments_number": installments}
        write_json(out_dir / "result.json", result)
        return
