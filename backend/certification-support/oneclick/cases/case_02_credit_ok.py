...
def main() -> None:
    ...
    out_dir = repo_root / "backend/qa-artifacts/transbank-cert/02_credit_ok" / datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                page.wait_for_url(
                    re.compile(r"https://webpay3gint\.transbank\.cl/webpayserver/dist/#/.*"),
                    timeout=60000,
                )
            except Exception:
                pass
            ...
            try:
                ...
                if tarjetas.count() > 0 and tarjetas.first.is_visible():
                    ...
                    page.screenshot(path=str(out_dir / "screenshots/00_after_tarjetas.png"), full_page=True)
            except Exception:
                pass
...
"""
                ...
                write_json(out_dir / "debug.inputs_seen.json", {"ts": now_iso(), "url": page.url, "inputs": inputs_seen})
            ...
            page.wait_for_selector("input[autocomplete='cc-exp'], #card-exp", timeout=60000, state="attached")
            page.wait_for_selector("input[autocomplete='cc-csc'], #card-cvv", timeout=60000, state="attached")

            ce.first.fill(exp)
            cc.first.fill(cvv)
            page.wait_for_timeout(500)
            page.screenshot(path=str(out_dir / "screenshots/01_after_card_fill.png"), full_page=True)
            ...
            page.wait_for_timeout(800)
            page.screenshot(path=str(out_dir / "screenshots/02_auth_page.png"), full_page=True)

            rut_norm = rut.replace(".", "")
            page.locator("#rutClient").first.fill(rut_norm)
            page.locator("#passwordClient").first.fill(password)
            page.wait_for_timeout(300)
            page.screenshot(path=str(out_dir / "screenshots/03_auth_filled.png"), full_page=True)

            page.locator("input[type='submit']").first.click(timeout=15000)

            try:
                page.wait_for_url(re.compile(r".*/api/payments/oneclick/confirm-return.*"), timeout=15000)
            except Exception:
                pass
            ...
            final_url = page.url
            if not tbk_user:
                m = re.search(r"[?&]tbk_user=([^&]+)", final_url)
                if m:
                    tbk_user = m.group(1)

            if callback_hits:
                callback_evidence = callback_hits[-1]

            page.wait_for_timeout(1200)
            page.screenshot(path=str(out_dir / "screenshots/04_after_return.png"), full_page=True)
            ...
            result["callback"] = callback_evidence
        ...
        result["error"] = f"{type(e).__name__}: {e}"[:300]
        try:
            page.screenshot(path=str(out_dir / "screenshots/FAIL_playwright.png"), full_page=True)
        except Exception:
            pass
        write_json(out_dir / "browser.console.json", {"console": console_logs, "page_errors": page_errors})
        write_json(out_dir / "browser.network.json", {"requests": requests_log, "responses": responses, "framenavigated": framenav})
    ...
    if not result.get("tbk_user"):
        result["failure_step"] = result["failure_step"] or "tbk_user not obtained"
        write_json(out_dir / "result.json", result)
        return

    cb = result.get("callback")
    if not cb or cb.get("status") not in (200, 302):
        result["failure_step"] = result["failure_step"] or "confirm-return not OK"
        write_json(out_dir / "result.json", result)
        return

    if cb.get("status") == 302:
        loc = (cb.get("headers") or {}).get("location")
        if not (isinstance(loc, str) and "tbk_user=" in loc):
            result["failure_step"] = result["failure_step"] or "confirm-return missing location"
            write_json(out_dir / "result.json", result)
            return

    if not (mongo_doc and mongo_doc.get("tbk_user") == result["tbk_user"] and mongo_doc.get("status") == "INSCRIBED"):
        result["failure_step"] = result["failure_step"] or "mongo tbk_user mismatch"
        write_json(out_dir / "result.json", result)
        return

    try:
        ok_event = False
        for ev in mongo_events or []:
            if ev.get("type") == "inscription_approved" and ev.get("detail", {}).get("tbk_user") == result["tbk_user"]:
                ok_event = True
                break
        if not ok_event:
            result["failure_step"] = result["failure_step"] or "missing inscription_approved event"
            write_json(out_dir / "result.json", result)
            return
    except Exception:
        pass
    ...
    update_typeform_answers(
        repo_root,
        "case_2",
        "tbk_user inscripción crédito aprobada",
        result["tbk_user"],
