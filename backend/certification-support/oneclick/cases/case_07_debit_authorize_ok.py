...
def main() -> None:
    ...
    run_id = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ") + "_case7_debit_authorize"
    out_dir = repo_root / "backend/qa-artifacts/transbank-cert/07_debit_authorize_ok" / run_id
    ...
    if not (rc == 0 and status == "AUTHORIZED"):
        result["failure_step"] = "authorize not approved"
    ...
    update_typeform_answers(
        repo_root,
        "case_7",
        "Authorize débito/prepago aprobado",
