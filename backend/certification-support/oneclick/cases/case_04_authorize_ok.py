...
def main() -> None:
    ...
    out_dir = repo_root / "backend/qa-artifacts/transbank-cert/04_authorize_ok" / datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    ...
    if not (response_code == 0 and status == "AUTHORIZED"):
        result["failure_step"] = "authorize not authorized"
    ...
    update_typeform_answers(
        repo_root,
        "case_4",
        "Authorize crédito aprobado (1 cuota)",
        {
