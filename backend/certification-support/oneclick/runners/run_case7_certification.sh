...
RUNS_DIR="$ROOT/backend/qa-artifacts/transbank-cert/07_debit_authorize_ok"
...
MISSING_PY_MODULE=""
if precheck_python >/tmp/_case7_missing_module.txt 2>/dev/null; then
  MISSING_PY_MODULE="$(cat /tmp/_case7_missing_module.txt 2>/dev/null || true)"
  rm -f /tmp/_case7_missing_module.txt >/dev/null 2>&1 || true
  echo FAIL
...
CASE7_STDOUT="$RUNS_DIR/_case7_stdout.log"
CASE7_STDERR="$RUNS_DIR/_case7_stderr.log"
"$PYTHON_BIN" "$ROOT/backend/certification-support/oneclick/cases/case_07_debit_authorize_ok.py" >"$CASE7_STDOUT" 2>"$CASE7_STDERR" || {
  echo FAIL
  echo failure_step=case7_script_failed
  exit 14
}
...
mv "$CASE7_STDOUT" "$RUNS_DIR/$RUN_ID/case7_stdout.log" >/dev/null 2>&1 || true
mv "$CASE7_STDERR" "$RUNS_DIR/$RUN_ID/case7_stderr.log" >/dev/null 2>&1 || true
...
if rc != 0 or st != "AUTHORIZED":
    print("FAIL"); print("failure_step=transbank_not_authorized"); sys.exit(23)
