...
RUNS_DIR="$ROOT/backend/qa-artifacts/transbank-cert/06_debit_ok"
...
MISSING_PY_MODULE=""
if precheck_python >/tmp/_case6_missing_module.txt 2>/dev/null; then
  MISSING_PY_MODULE="$(cat /tmp/_case6_missing_module.txt 2>/dev/null || true)"
  rm -f /tmp/_case6_missing_module.txt >/dev/null 2>&1 || true
  echo FAIL
...
CASE6_STDOUT="$RUNS_DIR/_case6_stdout.log"
CASE6_STDERR="$RUNS_DIR/_case6_stderr.log"
"$PYTHON_BIN" "$ROOT/backend/certification-support/oneclick/cases/case_06_debit_ok.py" >"$CASE6_STDOUT" 2>"$CASE6_STDERR" || {
  echo FAIL
  echo failure_step=case6_script_failed
  exit 14
}
...
mv "$CASE6_STDOUT" "$RUNS_DIR/$RUN_ID/case6_stdout.log" >/dev/null 2>&1 || true
mv "$CASE6_STDERR" "$RUNS_DIR/$RUN_ID/case6_stderr.log" >/dev/null 2>&1 || true
