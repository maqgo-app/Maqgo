...
async def save_oneclick_credentials(
    ...
    idempotency_key, key_legacy = resolve_idempotency_key(req, "save")
...
async def confirm_return(request: Request):
    ...
    try:
        ...
        if not tbk_user:
            await db[PAYMENTS_COLLECTION].update_one(
                {"token": token},
                {"$set": {"status": "FAILED", "updated_at": _now_iso(), "error": "sin_tbk_user"}},
            )
            return RedirectResponse(
        ...
        logger.info("Oneclick confirm OK buy_order=%s", payment.get("buy_order"))
...
async def confirm_inscription(request: Request, body: ConfirmInscriptionRequest | None = None):
    ...
    try:
        ...
        tbk_user = result.get("tbk_user", "")
        status = "INSCRIBED" if tbk_user else "FAILED"
        await db[PAYMENTS_COLLECTION].update_one(
            {"token": final_token},
            {
                "$set": {
...
async def authorize_payment(
    ...
    try:
        ...
        logger.info(
            "Oneclick authorize validation OK buy_order=%s exists=1 status_before=INSCRIBED approved=%s",
            data.buy_order,
            approved,
        )
        return result
        ...
        raise HTTPException(
            status_code=502,
            detail={"error": "transbank_error", "message": str(error_text), "buy_order": data.buy_order},
        )
...
@limiter.limit("30/minute")
async def debug_last_oneclick_token(_: dict = Depends(get_current_admin)):
    """
