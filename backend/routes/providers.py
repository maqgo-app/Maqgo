...
async def match_providers(
    ...
    try:
        ...
        max_price = max(p['price_per_hour'] for p in eligible_providers)
        max_distance = max(p['distance'] for p in eligible_providers) or 1
        
        for provider in eligible_providers:
            norm_price = provider['price_per_hour'] / max_price if max_price > 0 else 0
            norm_distance = provider['distance'] / max_distance if max_distance > 0 else 0
            
            total_decisions = provider['accepted_services'] + provider['rejected_services']
            if total_decisions > 0:
                acceptance_rate = provider['accepted_services'] / total_decisions
            else:
                # Neutral para proveedores nuevos sin historial
                acceptance_rate = 0.5
            acceptance_penalty = 1 - acceptance_rate  # más alto = peor

            rt = provider.get('response_time_avg')
            if rt is None:
                norm_response = 0.5  # neutro si no hay dato
            else:
                try:
                    rt_val = max(float(rt), 0.0)
                except (TypeError, ValueError):
                    rt_val = 60.0
                # Normalizar 0–60 min a 0–1 (más alto = peor)
                norm_response = min(rt_val / 60.0, 1.0)

            # Score final: menor es mejor
            provider['score'] = (
                0.45 * norm_price +
                0.25 * norm_distance +
                0.20 * acceptance_penalty +
                0.10 * norm_response
            )
        
        # Ordenar por score (menor es mejor)
        eligible_providers.sort(key=lambda x: x['score'])
        
        # Limitar resultados
        result_providers = eligible_providers[:limit]
        ...
        print(f"Error in match_providers: {e}")
        return {
            "providers": get_demo_providers(machinery_type, client_lat, client_lng),
            "total": 3,
            "is_demo": True,
