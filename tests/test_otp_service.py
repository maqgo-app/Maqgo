...
class TestOTPServiceSend:
    ...
    def test_send_otp_success(self, mock_send_sms, mock_redis):
        ...
        assert 'MAQGO - Código de verificación. Expira en 5 minutos.' in msg
        lines = msg.splitlines()
        assert len(lines) >= 3
        assert lines[0].isdigit() and len(lines[0]) == 6
        assert lines[1] == ""
    ...
    @patch('services.otp_service._get_redis', return_value=None)
    def test_send_otp_no_redis(self, _mock_redis):
        ...
        assert 'REDIS' in err.upper() or 'not configured' in err.lower()
