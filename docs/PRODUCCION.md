...
## Auth en MVP (decisión de alcance)

Para el **primer lanzamiento**, el criterio operativo es **login y recuperación vía celular (SMS)**. Se **mantiene** la regla: **recuperación de cuenta por SMS** como referencia de producto; el reenvío de OTP en login obedece a **riesgo** (dispositivo confiable o no, señales de contexto, fallos de verificación), no a una caducidad arbitraria por tiempo.

Los caminos de **correo** en recuperación de contraseña / OTP por email pueden existir en código pero se tratan como **secundarios o beta**: no bloquean el go-live; su validación completa (proveedor, dominio, anti‑spam) queda para después y está referenciada en `mvp-2.0/README.md`.
