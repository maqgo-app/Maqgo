...
2. **CNAME `api` → `maqgo-production.up.railway.app`**  
  La URL pública por defecto del servicio (`*.up.railway.app`) **no tiene por qué ser** el **destino del CNAME** que Railway exige para un **dominio personalizado** (`api2.maqgo.cl`).  
...
## Checklist mínimo (api2.maqgo.cl)

1. Railway → servicio → **Settings → Networking** → dominio `api2.maqgo.cl` → **Show DNS records**.
