...
const DevSessionRedirect = import.meta.env.DEV
  ? lazy(() => import('./screens/DevSessionRedirect.jsx'))
  : null;
...
function LegacyVerifiedToSelectRole() {
  ...
  try {
    ...
    if (desiredRole === 'provider') state = { preselect: 'provider' };
    else if (returnUrl && returnUrl.startsWith('/client/')) state = { preselect: 'client' };
  } catch {
    /* ignore */
  }
...
const MachinePhotosPricingScreen = lazy(() => import('./screens/provider/MachinePhotosPricingScreen'));
  ...
  return (
    ...
    <div className={`maqgo-app-shell-inner ${showBottomNav ? 'maqgo-with-bottom-nav' : ''}`}>
      ...
      <Routes>
        ...
        {DevSessionRedirect ? <Route path="/dev/session" element={<DevSessionRedirect />} /> : null}
        ...
        <Route
          ...
          element={
            <AppNavigateToLogin trace="src/App.jsx (Route path=/provider/verify-sms)" state={{ entry: 'provider' }} />
          }
        />
