 ...
 * cachés viejas o enlaces guardados no muestren UI obsoleta (OTP unificado en login / provider/register).
 */
(function redirectLegacySmsChannelPaths() {
  if (typeof window === "undefined") return;
  const raw = window.location.pathname || "";
  const path = raw.replace(/\/+$/, "") || "/";
  if (path === "/select-channel") {
    try {
      const dr = window.localStorage?.getItem("desiredRole");
      const target = dr === "provider" ? "/provider/register" : "/login";
      window.location.replace(
        `${window.location.origin}${target}${window.location.search}${window.location.hash}`
      );
    } catch {
  ...
  if (path === "/provider/select-channel") {
    try {
      const hasTok = Boolean(
        window.localStorage?.getItem("token") || window.localStorage?.getItem("authToken")
      );
      const hasUid = Boolean(window.localStorage?.getItem("userId"));
      const target = hasTok && hasUid ? "/provider/verified" : "/provider/register";
      window.location.replace(
        `${window.location.origin}${target}${window.location.search}${window.location.hash}`
      );
    } catch {
      window.location.replace(`${window.location.origin}/provider/register`);
    }
  }
})();
