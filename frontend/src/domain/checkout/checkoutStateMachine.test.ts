...
describe('checkoutReducer', () => {
  ...
  it('SERVICE_CONFIRMED + CONFIRM_SERVICE → SERVICE_CONFIRMED (idempotente al volver a P5)', () => {
    expect(checkoutReducer('SERVICE_CONFIRMED', { type: 'CONFIRM_SERVICE' })).toBe(
      'SERVICE_CONFIRMED'
    );
  });
  ...
  it('cualquier estado + RESET → IDLE', () => {
    const states = [
      'SERVICE_CONFIRMED',
      'CARD_CAPTURED',
      'PENDING_PROVIDER_ACCEPTANCE',
    ...
    expect(checkoutReducer('IDLE', { type: 'RESET' })).toBe('IDLE');
  });
});
