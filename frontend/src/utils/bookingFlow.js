 ...
 * Evita duplicación y rutas divergentes entre ProviderHomeScreen, WelcomeScreen y ClientHome.
 */

import { getAllClientCapacityStorageKeys } from './machineryNames.js';
import {
...
export function readClientBookingSnapshot() {
  ...
  const urgencyTypeSnap = localStorage.getItem('urgencyType') || '';
  ...
  const truckUrgency = isTruckUrgencyBooking(machinery);
  let hoursToday = Math.max(
    MIN_HOURS_IMMEDIATE,
    Math.min(MAX_HOURS_IMMEDIATE, savedHours)
  );
  // Camiones trip: multiplicador de pricing (4–8), no duración del servicio ni "5 horas" en UI.
  if (truckUrgency) {
    hoursToday = Math.max(
      MIN_HOURS_IMMEDIATE,
      Math.min(
        MAX_HOURS_IMMEDIATE,
        getTruckPricingHoursFromUrgency(urgencyTypeSnap)
      )
    );
  }
  ...
  return {
    ...
    additionalDays,
    isHybrid,
    serviceModel: truckUrgency ? 'truck' : 'hourly',
    urgencyType: truckUrgency ? urgencyTypeSnap : '',
    timeRange: truckUrgency ? getTruckTimeRangeFromUrgency(urgencyTypeSnap) : null,
  };
}
...
export const CLIENT_BOOKING_STORAGE_KEYS = [
  ...
  'clientBookingServiceType', 'truckUrgencyTimeRange', 'serviceModel', 'durationHours',
];
