...
/**
 * C04 - Verificación SMS con mejores prácticas UX
 * - Timer de expiración
 * - Cooldown de reenvío
 * - Web OTP API (auto-lectura SMS en Chrome)
 * - Copy-paste auto-complete
 */
function VerifySMSScreen() {
  ...
  const handleChange = (index, value) => {
    // Si pega código completo (6 dígitos)
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      try {
        ...
        // Evitar romper flujo con respuestas no-JSON.
      }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      try {
        ...
        // Evitar falsos errores de conexión por respuestas no-JSON.
      }

      if (response.ok && data?.success) {
        setExpiry(300); // Reiniciar timer
  ...
  return (
    <div className="maqgo-app">
      <div className="maqgo-screen">
        {/* Logo */}
        <MaqgoLogo size="small" style={{ marginBottom: 24 }} />
