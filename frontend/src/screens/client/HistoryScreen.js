...
function HistoryScreen() {
  ...
  const handleRepeat = (service) => {
    // Solo clientes pueden repetir
    if (isProvider) return;
    
    localStorage.setItem('selectedMachinery', service.machinery);
    localStorage.setItem('selectedHours', service.hours.toString());
  ...
  return (
    <div className="maqgo-app">
      <div className="maqgo-screen" style={{ padding: 'var(--maqgo-screen-padding-top) 20px 90px' }}>
              ...
              >
                ...
                {service.status === 'completed' && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    {/* Mi calificación */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ color: 'rgba(255,255,255,0.95)', fontSize: 11 }}>Mi calificación:</span>
        ...
        <button 
          className="maqgo-btn-primary"
          onClick={() => navigate(isProvider ? '/provider/home' : '/client/home')}
          style={{ marginTop: 8 }}
        >
