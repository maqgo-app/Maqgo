    ...
    <div className="maqgo-app maqgo-client-funnel maqgo-funnel-split-layout">
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2, marginBottom: 8 }}>
          {DAYS.map(day => (
            <div key={day} style={{ textAlign: 'center', color: 'rgba(255,255,255,0.95)', fontSize: 13, padding: 4 }}>
              {day}
            </div>
          ))}
        </div>
      ...
      <div className="maqgo-funnel-split-footer" role="region" aria-label="Continuar fecha">
        <button
          type="button"
          className="maqgo-btn-primary"
          onClick={handleContinue}
          disabled={!selectedDate}
          style={{ width: '100%', opacity: selectedDate ? 1 : 0.5 }}
        >
          CONTINUAR
        </button>
      </div>
